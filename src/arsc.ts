...
/**
 * Resolve a resource ID (e.g. 0x7f0d001b) to a string value from resources.arsc.
 * Returns null if the entry is missing or not a simple string value.
 */
export function resolveStringResource(data: Uint8Array, resourceId: number): string | null {
  ...
  if (view.byteLength < 12 || view.getUint16(0, true) !== RES_TABLE_TYPE) return null;

  const tableHeaderSize = view.getUint16(2, true);
  let cursor = tableHeaderSize;

  if (cursor + 8 > view.byteLength || view.getUint16(cursor, true) !== RES_STRING_POOL_TYPE) {
    return null;
  }
...
function findStringInPackage(
  ...
  while (innerCursor + 8 <= pkgBase + pkgSize) {
    ...
    if (innerSize <= 0 || innerCursor + innerSize > pkgBase + pkgSize) break;
    ...
    if (innerType === RES_TABLE_TYPE_TYPE) {
      ...
      if (thisTypeId === typeId && entryId < entryCount) {
        ...
        if (entryOffset !== NO_ENTRY) {
          ...
          // ResTable_entry is 8 bytes; Res_value follows it for simple entries.
          if (entryBase + 16 <= view.byteLength) {
            const valueType = view.getUint8(entryBase + 11);
            const valueData = view.getUint32(entryBase + 12, true);
            if (valueType === RES_VALUE_TYPE_STRING && valueData < globalStrings.length) {
              return globalStrings[valueData] || null;
            }
          }
        }
      }
